...
\begin{document}
	...
	\quote{dot -Tpdf myDotFile.dot $>$ myDotFile.pdf}\\
	...
	\quote{euncon reconcile:eun threshold:0 outfile:myOutput graphFile1.dot graphFile2.tre}
	
	
	\subsection{Execution in Parallel}
	By default the program will execute using a single process core.  By specifying the options `+RTS -NX -RTS' where `X' is the number of processors offered to the program. These are specified after the program as in :
	\quote{euncon +RTS -N4 -RTS }other options...  
